...
class GTFSManager:
    ...
    def create_stop_times(
        self, first_stop_id: int, last_stop_id: int, n_trips: int = 10
    ):
        ...
        reference_stop_times = stop_times_df[
            stop_times_df["trip_id"].isin(reference_trips)
        ]

        avg_speed_kmh = self.calculate_average_speed(
            reference_trips, reference_stop_times, stops_df
        )
        print(f"Average speed from reference trips: {avg_speed_kmh:.2f} km/h")

        # Create new stop times
        new_trips = trips_df[trips_df["route_id"] > REFERENCE_ROUTE_ID][
            "trip_id"
        ].unique()
        new_stops = stops_df[
        ...
        for trip_id in new_trips:
            ...
            current_time = datetime.strptime(
                ref_trip.iloc[0]["arrival_time"], "%H:%M:%S"
            )

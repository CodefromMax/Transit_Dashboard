...
class CalculationPipeline():
    ...
    def combine_metrics_tables(self):
        ...
        # File paths for travel time to first destination and job access (unchanged)
        ...
        # Define a new function to process destination datasets that vary by threshold
        ...
        # Existing function for travel time to first destination metrics (unchanged)
        ...
        # Process the destination datasets.
        # Define the thresholds and categories. We assume that for each category there are four files.
        ...
        for cat, file_suffix in categories.items():
            ...
            # Combine the four thresholds for this category
            dest_cat_df = pd.concat(dest_dfs, ignore_index=True)
            all_destination_rows.append(dest_cat_df)

            # Pivot the data so that each row (grouped by CTUID, before/after, etc.) has values for each threshold
            ...
            # Compute the incremental differences
            # For 30, 45, and 60 thresholds, compute difference from the previous threshold.
            deltas = []
            for high, low in [(30, 15), (45, 30), (60, 45)]:
                temp = pivot_df.copy()
                temp["Value"] = temp[high] - temp[low]
                temp["Travel_Time_Threshold"] = high
                # Change the metric name to indicate incremental value
                temp["Metric_Name"] = "Number of Destinations Accessible (Incremental)"
                temp = temp[pivot_cols + ["Travel_Time_Threshold", "Value"]]
                deltas.append(temp)
            # Add these delta rows to our destination rows list
        ...
        # Process travel time to first destination datasets (unchanged)
